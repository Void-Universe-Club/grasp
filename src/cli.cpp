...
void print_outcomes(const std::vector<StepOutcome>& outs) {
    for (size_t i = 0; i < outs.size(); ++i) {
        ...
        std::cout << "== node " << o.node << " [" << o.kind << "]: \""
                  << os::trunc_utf8(o.desc, 120) << "\"\n";
        std::cout << "   output: \"" << os::trunc_utf8(o.output, 200) << "\"\n";
...
int cmd_list(const std::vector<std::string>& args, std::string*) {
    std::vector<Session> all = store().list();
    // --summary: JSON global state (full picture for the agent, 2026-08-24)
    if (std::find(args.begin(), args.end(), "--summary") != args.end()) {
        nlohmann::json out = nlohmann::json::array();
        for (size_t i = 0; i < all.size(); ++i) {
            const Session& s = all[i];
            int refuted = 0, confirmed = 0, pending = 0, unexpl = 0;
            for (const Node& nd : s.graph.nodes) {
                if (nd.result == "refuted") ++refuted;
                else if (nd.result == "confirmed") ++confirmed;
                else if (nd.result.empty() || nd.result == "pending") ++pending;
            }
            for (const Edge& e : s.graph.edges)
                if (s.graph.edge_visit_count(e.from, e.to) == 0) ++unexpl;
            nlohmann::json o;
            o["id"] = s.id;
            o["graph"] = s.graph.id;
            o["v"] = s.graph.version;
            o["nodes"] = s.graph.nodes.size();
            o["edges"] = s.graph.edges.size();
            o["unexplored"] = unexpl;
            o["refuted"] = refuted;
...
int cmd_walk(const std::vector<std::string>& args, std::string* current) {
    ...
    // --json: structured output (consumed directly by the agent, 2026-08-24)
    if (std::find(flags.begin(), flags.end(), "json") != flags.end()) {
        const Graph& g = s.graph;
        std::string cur = !from.empty() ? from : (s.started() ? s.node : g.entry);
        const Node* cn = g.find_node(cur);
        nlohmann::json out;
        out["cur"] = cur;
        if (cn) out["desc"] = os::trunc_utf8(cn->desc, 120);
        out["options"] = nlohmann::json::array();
        for (const Edge* e : g.edges_from(cur)) {
            nlohmann::json o;
            o["to"] = e->to;
            if (!e->label.empty()) o["label"] = e->label;
            const Node* tn = g.find_node(e->to);
            if (tn) o["desc"] = os::trunc_utf8(tn->desc, 60);
            o["visited"] = g.edge_visit_count(cur, e->to) > 0;
            o["visits"] = g.edge_visit_count(cur, e->to);
...
int cmd_insert(const std::vector<std::string>& args, std::string* current) {
    ...
    std::string sid = session_id_of(rest, &pos, current);
    nlohmann::json j;
    bool use_stdin = std::find(flags.begin(), flags.end(), "stdin") != flags.end();
    if (use_stdin) {  // read node JSON from stdin (avoid UTF-8 arg escaping, 2026-08-24)
        std::stringstream buf;
        buf << std::cin.rdbuf();
        j = nlohmann::json::parse(buf.str());
    } else if (opts.count("file") > 0) {  // read the node JSON from a file (long descs stay readable)
        ...
        if (opts.count("hypothesis") > 0) j["hypothesis"] = opts["hypothesis"];
        if (opts.count("result") > 0) j["result"] = opts["result"];
        if (opts.count("evidence") > 0) j["evidence"] = opts["evidence"];
        if (opts.count("files") > 0) j["related_files"] = opts["files"];
    }
...
// ---------- exploration-augmentation commands (2026-08-24) ----------

// mark: manually flag nodes/edges as explored (insert/add-edge auto-mark; for legacy graphs)
int cmd_mark(const std::vector<std::string>& args, std::string* current) {
    std::map<std::string, std::string> opts;
    std::vector<std::string> flags;
    std::vector<std::string> rest = parse_options(args, 1, opts, &flags);
    size_t pos = 0;
    std::string sid = session_id_of(rest, &pos, current);
    if (pos >= rest.size()) {
        throw std::runtime_error("usage: mark <session-id> <node-id> [--edge from,to]");
    }
    Session s = store().load(sid);
    if (opts.count("edge") > 0) {
        std::string spec = opts["edge"];
        size_t comma = spec.find(',');
        if (comma == std::string::npos) throw std::runtime_error("--edge must be from,to");
        s.graph.mark_edge_visited(spec.substr(0, comma), spec.substr(comma + 1));
        std::cout << "marked edge " << spec << " visited\n";
    }
    s.graph.mark_visited(rest[pos]);
    store().save(s);
    std::cout << "marked node " << rest[pos] << " visited | graph v" << s.graph.version << "\n";
    return 0;
}

// query: graph queries --unexplored | --path A,B | --subtree X | --refuted | --pending
int cmd_query(const std::vector<std::string>& args, std::string* current) {
    std::map<std::string, std::string> opts;
    std::vector<std::string> flags;
    std::vector<std::string> rest = parse_options(args, 1, opts, &flags);
    size_t pos = 0;
    ...
    if (std::find(flags.begin(), flags.end(), "unexplored") != flags.end()) {
        int n = 0;
        ...
        std::cout << n << " unexplored edge(s)\n";
        return 0;
    }
    if (opts.count("path") > 0) {
        std::string spec = opts["path"];
        size_t comma = spec.find(',');
        if (comma == std::string::npos) throw std::runtime_error("--path must be A,B");
        std::string from = spec.substr(0, comma), to = spec.substr(comma + 1);
        std::map<std::string, std::string> prev;
        std::vector<std::string> queue = {from};
        prev[from] = "";
        bool found = false;
        for (size_t qi = 0; qi < queue.size() && !found; ++qi) {
            for (const Edge* e : g.edges_from(queue[qi])) {
                if (prev.count(e->to)) continue;
                prev[e->to] = queue[qi];
                if (e->to == to) { found = true; break; }
                queue.push_back(e->to);
            }
        }
        if (!found) { std::cout << "no path " << from << " -> " << to << "\n"; return 0; }
        std::vector<std::string> path;
        for (std::string c = to; !c.empty(); c = prev[c]) path.push_back(c);
    ...
    if (opts.count("subtree") > 0) {
        std::string root = opts["subtree"];
    ...
    if (std::find(flags.begin(), flags.end(), "refuted") != flags.end()) {
        int n = 0;
        ...
        std::cout << n << " refuted node(s)\n";
        return 0;
    }
    if (std::find(flags.begin(), flags.end(), "pending") != flags.end()) {
        int n = 0;
        for (const Node& nd : g.nodes) {
        ...
        std::cout << n << " pending node(s)\n";
        return 0;
    }
    throw std::runtime_error("query: --unexplored | --path A,B | --subtree X | --refuted | --pending");
}

// diff: compare current with a version backup (sessions/<id>.json.v<N>)
int cmd_diff(const std::vector<std::string>& args, std::string* current) {
    std::map<std::string, std::string> opts;
    std::vector<std::string> flags;
    std::vector<std::string> rest = parse_options(args, 1, opts, &flags);
    size_t pos = 0;
    std::string sid = session_id_of(rest, &pos, current);
    if (pos >= rest.size()) {
        throw std::runtime_error("usage: diff <session-id> <version>  (compare current with v<version> backup)");
    }
    Session cur = store().load(sid);
    std::string bak_path = sessions_dir() + "/" + sid + ".json.v" + rest[pos];
    std::ifstream in(bak_path.c_str());
    if (!in) throw std::runtime_error("no backup " + bak_path);
    std::stringstream buf; buf << in.rdbuf();
    nlohmann::json j = nlohmann::json::parse(buf.str());
    Session old = j.get<Session>();
    std::cout << "== diff v" << rest[pos] << " -> v" << cur.graph.version << " ==\n";
    std::cout << "new nodes:\n";
    for (const Node& n : cur.graph.nodes) {
        if (!old.graph.find_node(n.id)) std::cout << "  + " << n.id << "\n";
    }
    std::cout << "new edges:\n";
    for (const Edge& e : cur.graph.edges) {
        bool found = false;
        for (const Edge& oe : old.graph.edges)
            if (oe.from == e.from && oe.to == e.to) { found = true; break; }
        if (!found) std::cout << "  + " << e.from << " -> " << e.to << "\n";
    }
    return 0;
}

// context: session context bundle (knowledge injected at agent-session start, 2026-08-24)
// dense: reachable-node summaries + unexplored edges + validated-conclusion list
int cmd_context(const std::vector<std::string>& args, std::string* current) {
    std::map<std::string, std::string> opts;
    std::vector<std::string> flags;
    std::vector<std::string> rest = parse_options(args, 1, opts, &flags);
    size_t pos = 0;
    ...
    Session s = store().load(sid);
    const Graph& g = s.graph;
    std::string entry = g.entry.empty() ? (g.nodes.empty() ? "" : g.nodes[0].id) : g.entry;
    std::cout << "== context " << sid << " (graph " << g.id << " v" << g.version
    ...
    for (const Edge& e : g.edges)
        if (g.edge_visit_count(e.from, e.to) == 0) ue.push_back(e.from + "->" + e.to);
    std::cout << "\n-- unexplored (" << ue.size() << "): ";
    for (size_t i = 0; i < ue.size(); ++i) std::cout << (i ? ", " : "") << ue[i];
    std::cout << "\n";
    // 4. validated conclusions
    int nv = 0;
    std::cout << "-- validation:";
    for (const Node& nd : g.nodes) {
...
// update: edit node fields in place (meta visit stats preserved, 2026-08-24)
int cmd_update(const std::vector<std::string>& args, std::string* current) {
    std::map<std::string, std::string> opts;
    std::vector<std::string> flags;
    std::vector<std::string> rest = parse_options(args, 1, opts, &flags);
    size_t pos = 0;
    ...
    if (n == NULL) throw std::runtime_error("node '" + rest[pos] + "' not found");
    int changed = 0;
    if (opts.count("desc") > 0) { n->desc = opts["desc"]; ++changed; }
    if (opts.count("result") > 0) { n->result = opts["result"]; ++changed; }
    if (opts.count("evidence") > 0) { n->evidence = opts["evidence"]; ++changed; }
    if (opts.count("hypothesis") > 0) { n->hypothesis = opts["hypothesis"]; ++changed; }
    ...
    s.graph.version++;
    store().save(s);
    std::cout << "updated node " << rest[pos] << " (" << changed << " fields) | graph v"
              << s.graph.version << "\n";
    return 0;
}
...
int cmd_dispatch(const std::vector<std::string>& args, std::string* current_session) {
    ...
    try {
        ...
        if (cmd == "mark") return cmd_mark(args, current_session);
        if (cmd == "query") return cmd_query(args, current_session);
        if (cmd == "diff") return cmd_diff(args, current_session);
        if (cmd == "context") return cmd_context(args, current_session);
        if (cmd == "update") return cmd_update(args, current_session);
        ...
        if (cmd == "help") {
            std::cout <<
                ...
                "                                   the start node (or its first fork after a single-edge start);\n"
