...
StepOutcome session_execute_node(Session& s, const Node& n, SessionStore* store) {
    ...
    std::string output;
    if (n.kind == "exec") {
        output = os::utf8_sanitize(os::run_shell(n.cmd, n.timeout_secs));
    } else if (n.kind == "ask") {
...
std::string session_walk(Session& s, const std::string& from, int choose,
  ...
  // 1. append the node description (truncated for dense output; full text via show)
        if (!first) path << " → ";
        path << os::trunc_utf8(n->desc, 120);
        first = false;
  ...
  // (choose applies at the start node of this call -- resuming a stopped walk
  //  starts at that fork, so `walk sid --choose N` answers the pending question)
  ...
  // never-walked edges are marked [unexplored] to surface unexplored edges
            ...
            for (size_t i = 0; i < es.size(); ++i) {
                ...
                if (s.graph.edge_visit_count(cur, es[i]->to) == 0) {
                    ...
                    stmt << "(x" << s.graph.edge_visit_count(cur, es[i]->to) << ")";
                }
            }
...
void session_insert(Session& s, const Node& n, const Edge* e) {
    ...
    if (e != NULL) {
        ...
        // insert marks visited: new nodes and edges count as visited (2026-08-24)
        s.graph.mark_edge_visited(e->from, e->to);
    }
    s.graph.mark_visited(n.id);
    s.graph.version++;
}
...
void session_add_edge(Session& s, const Edge& e) {
    ...
    if (!err.empty()) throw std::runtime_error(err);
    s.graph.edges.push_back(e);
    // add-edge marks visited: new edges count as visited (2026-08-24)
    s.graph.mark_edge_visited(e.from, e.to);
    s.graph.version++;
}
...
// expand [[sid:node]] cross-session references in a description (2026-08-24)
std::string expand_refs(const std::string& desc, SessionStore& store) {
    std::string out = desc;
    size_t p = 0;
    while ((p = out.find("[[", p)) != std::string::npos) {
        size_t q = out.find("]]", p);
        if (q == std::string::npos) break;
        std::string ref = out.substr(p + 2, q - p - 2);
        std::string replacement = ref;
        size_t colon = ref.find(':');
        if (colon != std::string::npos) {
            std::string rsid = ref.substr(0, colon), rnode = ref.substr(colon + 1);
            try {
        ...
        out.replace(p, q - p + 2, replacement);
        p += replacement.size();
    }
    return out;
}
