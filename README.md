...
| `walk <sid> [--from ID] [--choose N] [--auto] [--auto N] [--steps N] [--json]` | stroll: stitch descriptions into a sentence; multi-edge nodes stop and ask; `--choose N` picks option N at the start node of the call (resume a stopped fork the same way); `--auto` prefers unexplored edges (else fallback, else first); `--auto N` picks option N at every fork (non-interactive); `--json` prints `{cur,desc,options[]}` |
